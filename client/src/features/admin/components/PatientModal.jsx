...
export const PatientModal = ({ isOpen, onClose, patient, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in-up">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-100">
            ...
            <form id="edit-patient-form" onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm"
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm cursor-pointer"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm resize-none"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows="2"
                  placeholder="e.g., Ayurvedic formulations (or leave blank)"
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm resize-none"
                />
              </div>
            </form>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 border-b border-slate-100 pb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">ID: {patient._id}</p>
                </div>
              </div>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 text-sm">
                    <div>
                      ...
                      <p className="text-slate-800 font-semibold">{patient.mobile}</p>
                      {patient.email && (
                    ...
                    <div>
                      ...
                      <p className="text-slate-700">
                        Gender: <span className="font-semibold">{patient.gender || 'N/A'}</span>
                      </p>
                      <p className="text-slate-700">
                        DOB:{' '}
                ...
                <div className="mt-4 space-y-4 max-h-80 overflow-y-auto pr-2">
                  {loadingAppts ? (
                    ...
                    appointments.map((appt) => (
                      <div
                        key={appt._id}
                        className="p-4 border border-slate-200 rounded-2xl bg-slate-50 flex flex-col sm:flex-row justify-between gap-4"
                      >
                        <div>
                          ...
                          <p className="text-sm text-slate-600">
                            Service: <span className="font-medium">{appt.preferredService}</span>
                          </p>
                          <p className="text-sm text-slate-600">
                            Consultation:{' '}
                            <span className="font-medium">{appt.consultationType}</span>
                          </p>
                        </div>
        ...
        <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/60">
          {isEditing ? (
            <>
              <Button variant="secondary" onClick={() => setIsEditing(false)}>
                Cancel
              </Button>
              <Button type="submit" form="edit-patient-form" variant="primary">
                Save Changes
              </Button>
            </>
          ) : (
